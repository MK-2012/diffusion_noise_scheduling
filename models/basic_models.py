...
class MovMNISTModel(Module):
    ...
    def __init__(self, sample_size, in_channels, out_channels, layers_per_block, block_out_channels, norm_num_groups, down_block_types, up_block_types, cross_attention_dim, attention_head_dim, model_type="image", class_num=0):
        super().__init__()
        ...
        if self.use_cond:
            self.cond_model = Embedding(num_embeddings=class_num, embedding_dim=cross_attention_dim)
        else:
    ...
    def _sin_embeddings(self, shape):
        ...
        match self.model_type:
            case "video":
                return sin(arange(1, shape[2] + 1, device=dev).view(-1, 1) * arange(1, self.cross_att_dim + 1, device=dev)).tile(shape[0]).view(shape[0], shape[2], -1)
            case "image":
                return sin(arange(1, self.cross_att_dim + 1, device=dev)).tile(shape[0]).view(shape[0], 1, -1)
    ...
    def forward(self, X, t, classes=None):
        ...
        return self.main_model.forward(sample=X, timestep=t, encoder_hidden_states=embs)
