...
class CustomEulerDiscreteSchedulerOutput():
    """
...
class CustomEulerDiscreteScheduler():
    """
    Euler scheduler.
    ...
    def _convert_to_karras(self, in_sigmas: torch.FloatTensor, num_inference_steps) -> torch.FloatTensor:
        ...
        sigma_min = self.sigma_min if self.sigma_min is not None else in_sigmas[-1].item()
        sigma_max = self.sigma_max if self.sigma_max is not None else in_sigmas[0].item()
    ...
    def step(
    ...
    ) -> Union[CustomEulerDiscreteSchedulerOutput, Tuple]:
        """
        ...
        noise = self.noise_sampler.sample(model_output.shape).to(model_output.device)
        ...
        return CustomEulerDiscreteSchedulerOutput(prev_sample=prev_sample, pred_original_sample=pred_original_sample)
