import numpy as np
from torch import eye, ones, zeros, arange, diagonal, einsum, diag_embed, randn, sqrt, pow
...
class NormalVideoNoise:
	...
	def sample(self, shape):
		return einsum("aj...,ij->ai...", randn(shape), self.V) + \
			   self.a.repeat(shape[2:] + [1]).permute(-1, *np.arange(len(shape) - 2))
